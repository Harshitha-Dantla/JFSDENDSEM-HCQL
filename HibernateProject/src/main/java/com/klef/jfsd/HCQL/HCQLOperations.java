...
public class HCQLOperations 
{
	...
	public void aggregatefunctions() 
	{
        ...
        h.select(c.max(root2.get("salary")));
        Double max=session.createQuery(h).getSingleResult();
        System.out.println("Maximum faculty salary:="+max);
        ...
        cq1.select(cb1.min(root3.get("salary")));
        Double min=session.createQuery(cq1).getSingleResult();
        System.out.println("Minimum faculty Salary:="+min);
        ...
        cq2.select(cb2.avg(root0.get("salary")));
        Double avg=session.createQuery(cq2).getSingleResult();
        System.out.println("Total faculty average:="+avg);
